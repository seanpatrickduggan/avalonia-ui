...
public sealed class SqliteWorkspaceDb : IWorkspaceDb
{
    ...
    public async Task InitializeAsync(string dbPath, CancellationToken ct = default)
    {
        ...
        {
            Log.Debug(ex, "Failed to detect schema version, treating as new database");
            hasSchemaInfo = false;
            existingVer = -1;
        }
        ...
        if (!baseFileExisted || !hasSchemaInfo)
        {
            ...
            {
                Log.Debug(ex, "WAL checkpoint failed during database creation");
            }
        ...
        // This database stores diagnostic/logging data (sessions, operations, log entries) that can be
        // regenerated by running operations again. Rather than implementing complex migration scripts,
        // we use a "recreate on mismatch" strategy: if the schema version doesn't match, we delete
        // the old database and create a fresh one. A notice file is written to inform users.
        // This is intentional - the workspace database is not intended for long-term data preservation.
        // If you need to preserve logs across schema changes, export them first using MaterializeOperationLogsAsync.
        if (existingVer != CurrentSchemaVersion)
        {
            try { await _conn.CloseAsync(); }
            catch (Exception ex) { Log.Debug(ex, "Failed to close connection before schema migration"); }
            _conn.Dispose();
            try { if (_fs.FileExists(dbPath)) _fs.DeleteFile(dbPath); }
            catch (Exception ex) { Log.Warning(ex, "Failed to delete old database file during schema migration at {DbPath}", dbPath); }
            ...
            {
                Log.Debug(ex, "WAL checkpoint failed during schema migration");
            }
            ...
            catch (Exception ex) { Log.Debug(ex, "Failed to write schema reset notice file at {NoticePath}", noticePath); }
            Log.Warning(msg);
            return;
        }
        ...
        {
            Log.Debug(ex, "WAL checkpoint failed during initialization");
        }
    }
